...
public:
  //constructor
  QConsole(QWidget *parent = nullptr, const QString &welcomeText = "");
  ...
  bool execCommand(const QString &command, bool writeCommand = true,
                   bool showPrompt = true, QString *result = nullptr);
  //saves a file script
...
protected:
  ...
  // Current history index (needed because afaik QStringList does not have
  // such an index)
  int historyIndex;
...
protected:
  ...
  // execute a validated command (should be reimplemented and called at the
  // end)
