...
MSGViewer::MSGViewer(int ac, const char **&av)
      ...
      rendererType("obj")
{
    ...
    if (msgModel->camera.size() > 0) {
      setViewPort(msgModel->camera[0]->from,
                  msgModel->camera[0]->at,
                  msgModel->camera[0]->up);
    }
}

void MSGViewer::parseCommandLine(int ac, const char **&av)
{
...
void MSGViewer::reportParsedData()
{
    // -------------------------------------------------------
    // done parsing
    // -------------------------------------------------------]
...
void MSGViewer::keypress(char key, const vec2f where)
{
    ...
    case 'S':
        ...
        ospSet1i(m_renderer,"shadowsEnabled",doShadows);
        ospCommit(m_renderer);
        accumID=0;
        ospFrameBufferClear(m_fb,OSP_FB_ACCUM);
        forceRedraw();
...
void MSGViewer::display()
{
    ...
    if (g_benchFrames > 0 && frameID== g_benchWarmup+g_benchFrames)
    {
        ...
        printf("Benchmark: time: %f avg fps: %f avg frame time: %f\n", time,
               avgFps, time/double(frameID-g_benchWarmup));
    ...
    if (outFileName) {
        ospSet1i(m_renderer,"spp",numSPPinFileOutput);
        ospCommit(m_renderer);
    ...
    ospRenderFrame(m_fb,m_renderer,OSP_FB_COLOR|(showDepthBuffer?OSP_FB_DEPTH:0)|OSP_FB_ACCUM);
    ++accumID;
...
OSPMaterial ospray::MSGViewer::createDefaultMaterial(OSPRenderer renderer)
{
...
OSPMaterial MSGViewer::createMaterial(OSPRenderer renderer, miniSG::Material *mat)
{
    if (mat == NULL) {
        static int numWarnings = 0;
        if (++numWarnings < 10)
