...
OSPObject *OSPObjectFile::importObjects()
{
  ...
  exitOnCondition(result != tinyxml2::XML_SUCCESS,
                  "unable to read object file '" + filename + "'");
  ...
  for (auto *node = xml.FirstChild(); node; node = node->NextSibling()) {
    if (node->ToElement()) {
      objects.push_back(importObject(node));
    }
  }

  // Copy the objects into a list.
  OSPObject *pointer = new OSPObject[objects.size() + 1];
  memcpy(pointer, &objects[0], objects.size() * sizeof(OSPObject));

  // Mark the end of the list.
  pointer[objects.size()] = NULL;  return(pointer);
}

void OSPObjectFile::importAttributeFloat(const tinyxml2::XMLNode *node,
                                         OSPObject parent)
{
  ...
  auto result = sscanf(text, "%f %7s", &value, guard);
  exitOnCondition(result != 1,
...
void OSPObjectFile::importAttributeFloat2(const tinyxml2::XMLNode *node,
                                          OSPObject parent)
{
  // The attribute value is encoded in a string.
  const char *text = node->ToElement()->GetText();  
  ...
  // Get the attribute value.
  auto result = sscanf(text, "%f %f %7s", &value.x, &value.y, guard);
  exitOnCondition(result != 2,
                  "malformed XML element '"
                  + std::string(node->ToElement()->Name()) + "'");
...
void OSPObjectFile::importAttributeFloat3(const tinyxml2::XMLNode *node,
                                          OSPObject parent)
{
  // The attribute value is encoded in a string.
  const char *text = node->ToElement()->GetText();  
  ...
  // Get the attribute value.
  auto result = sscanf(text,
                       "%f %f %f %7s",
                       &value.x,
                       &value.y,
                       &value.z,
                       guard);
  exitOnCondition(result != 3,
                  "malformed XML element '"
                  + std::string(node->ToElement()->Name()) + "'");
...
void OSPObjectFile::importAttributeInteger(const tinyxml2::XMLNode *node,
                                           OSPObject parent)
{
  // The attribute value is encoded in a string.
  const char *text = node->ToElement()->GetText();
  ...
  // Get the attribute value.
  auto result = sscanf(text, "%d %7s", &value, guard);
  exitOnCondition(result != 1,
                  "malformed XML element '"
                  + std::string(node->ToElement()->Name()) + "'");
...
void OSPObjectFile::importAttributeInteger3(const tinyxml2::XMLNode *node,
                                            OSPObject parent)
{
  // The attribute value is encoded in a string.
  const char *text = node->ToElement()->GetText();  
...
void OSPObjectFile::importAttributeString(const tinyxml2::XMLNode *node,
                                          OSPObject parent)
{
  // Get the attribute value.
  ...
  // Error check.
  exitOnCondition(strlen(value) == 0,
                  "malformed XML element '"
                  + std::string(node->ToElement()->Name())
                  + "'");
...
OSPObject OSPObjectFile::importObject(const tinyxml2::XMLNode *node)
{
  ...
  if (!strcmp(node->ToElement()->Name(), "light")) {
    return((OSPObject) importLight(node));
  }

  // OSPRay triangle mesh object.
  if (!strcmp(node->ToElement()->Name(), "triangleMesh")) {
    return((OSPObject) importTriangleMesh(node));
  }
...
OSPLight OSPObjectFile::importLight(const tinyxml2::XMLNode *root)
{
  ...
  for (auto *node = root->FirstChild(); node; node = node->NextSibling()) {
    ...
    // Light color.
    if (!strcmp(node->ToElement()->Name(), "color")) {
      importAttributeFloat3(node, light);
      continue;
    }

    // Light direction.
    if (!strcmp(node->ToElement()->Name(), "direction")) {
      importAttributeFloat3(node, light);
      continue;
    }

    // Light half angle for spot lights.
    if (!strcmp(node->ToElement()->Name(), "halfAngle")) {
      importAttributeFloat(node, light);
      continue;
    }

    // Light position.
    if (!strcmp(node->ToElement()->Name(), "position")) {
      importAttributeFloat3(node, light);
      continue;
    }

    // Light illumination distance cutoff.
    if (!strcmp(node->ToElement()->Name(), "range")) {
      importAttributeFloat(node, light);
      continue;
    }
...
OSPGeometry OSPObjectFile::importTriangleMesh(const tinyxml2::XMLNode *root)
{
  ...
  for (auto *node = root->FirstChild(); node; node = node->NextSibling()) {
    ...
    if (!strcmp(node->ToElement()->Name(), "filename")) {
      triangleMeshFilename = node->ToElement()->GetText();
      continue;
    }

    // Scaling for vertex coordinates.
    if (!strcmp(node->ToElement()->Name(), "scale")) {
      importAttributeFloat3(node, triangleMesh);
      continue;
    }
  ...
  if (triangleMeshFilename != NULL) {
    ...
    if (triangleMeshFilename[0] != '/') {
      std::string file = std::string(dirname(duplicateFilename))
                         + "/"
                         + triangleMeshFilename;
      return TriangleMeshFile::importTriangleMesh(file.c_str(), triangleMesh);
    }
...
OSPVolume OSPObjectFile::importVolume(const tinyxml2::XMLNode *root)
{
  ...
  if (dpFromEnv) {
    ...
    std::cout << "#osp.loader: found OSPRAY_DATA_PARALLEL env-var, "
              << "#osp.loader: trying to use data _parallel_ mode..."
              << std::endl;
    osp::vec3i blockDims;
    int rc = sscanf(dpFromEnv,
                    "%dx%dx%d",
                    &blockDims.x,
                    &blockDims.y,
                    &blockDims.z);
   ...
   ospSetVec3i(volume,"num_dp_blocks",blockDims);
    ...
    std::cout << "#osp.loader: no OSPRAY_DATA_PARALLEL dimensions set, "
              << "#osp.loader: assuming data replicated mode is desired"
              << std::endl;
    std::cout << "#osp.loader: to use data parallel mode,"
              << " set OSPRAY_DATA_PARALLEL env-var to <X>x<Y>x<Z>"
              << std::endl;
    std::cout << "#osp.loader: where X, Y, and Z are the desired _number_ of"
  ...
  for (auto *node = root->FirstChild(); node; node = node->NextSibling()) {
    ...
    if (!strcmp(node->ToElement()->Name(), "dimensions")) {
      importAttributeInteger3(node, volume);
      continue;
    }
    ...
    if (!strcmp(node->ToElement()->Name(), "gridOrigin")) {
      importAttributeFloat3(node, volume);
      continue;
    }
    ...
    if (!strcmp(node->ToElement()->Name(), "samplingRate")) {
      importAttributeFloat(node, volume);
      continue;
    }
    ...
    if (!strcmp(node->ToElement()->Name(), "voxelRange")) {
      importAttributeFloat2(node, volume);
      continue;
    }

    // Voxel type string.
    if (!strcmp(node->ToElement()->Name(), "voxelType")) {
      importAttributeString(node, volume);
      continue;
    }
  ...
  if (volumeFilename != NULL) {
    ...
    if (volumeFilename[0] != '/') {
      std::string file = std::string(dirname(duplicateFilename))
                         + "/" + volumeFilename;
      return(VolumeFile::importVolume(file, volume));
    }
...
std::string OSPObjectFile::toString() const
{
  return("ospray_module_loaders::OSPObjectFile");
}

OSPObjectFile::OSPObjectFile(const std::string &filename) :
  filename(filename)
{
}
